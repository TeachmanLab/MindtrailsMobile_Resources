...
def compression(short=True):
    ...
    others = []
    if short:
        name = "/HTC/csv_files/HTC_scenarios.csv"
    else:
        name = "/HTC/csv_files/HTC_long_scenarios.csv"
    if short:
        i_list = [9, 15, 21, 27]
    else:
        i_list = [5, 22, 39, 56]
    ...
    with open(name, newline='') as csvfile:
        ...
        for row in reader:
            ...
            # if label in total or label + "_undergrad" in total or label + "_grad" in total:

            if not(row[i_list[0]].strip() == row[i_list[1]].strip() == row[i_list[2]].strip() == row[i_list[3]].strip()):  # if they're not all equal
                print('not all equal')
                for group in groups:
                    print('got here')
                    if short:
                ...
                if "https://drive.google.com/" in gdrive_link:
                    ...
                    file_id = gdrive_link[start + 7:end]
                    url = "https://drive.google.com/uc?export=download&id=" + file_id
                    destination = '/HTC/images/' + pic_name + '.jpeg'
                    pic_counter += 1
                    message = "download url for pic" +pic_name+ " is " + url
                    print(message)
                    print("normal url for pic", pic_name, "is", gdrive_link)
    ...
    with open(name, newline='') as csvfile:
        ...
        for pic_name in total:
            try:
                path = '/HTC/images/' + pic_name + '.jpeg'
                image = Image.open(path)
                image.thumbnail((1300, 2600)) # 640
                image.save('/HTC/new_images/long_images_new/' + pic_name + '.jpeg')
            ...
            try:
                ...
                with open("/HTC/new_images/long_images_new/" + pic_name + ".jpeg", 'rb') as f:
                    content = f.read()
                git_file = "HTC/protocols/protocol1/media/images/" + pic_name + ".jpeg"
                # try:
