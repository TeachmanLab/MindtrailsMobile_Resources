import csv
import json
from MTM_helpers import get_lessons_learned_text
import random

file_path = r"/Users/xinyinzhang/Desktop/PACT Lab Training/HTC_csv/lessons_learned_text.csv"

lessons_learned_dict = get_lessons_learned_text(file_path)


def create_long_scenario_page_group(label, scenario_description, thoughts_lst, feelings_lst, behaviors_lst):
    """
    ...
    with open(r"/Users/xinyinzhang/Desktop/PACT Lab Training/MTM_csv/MTM_long_scenarios_structure.csv",
        ...
        for row_str in csv_reader:
            ...
            image_url = "https://github.com/TeachmanLab/MindtrailsMobile_Resources/raw/main/HTC/protocols/MTM/media/" \
                        + label.strip().replace(" ", "_") + ".jpeg"
            ...
            if row_str[13] not in (None, ""):
                page["Timeout"] = int(row_str[13]) #changing the string number from csv to an int
...
def create_scenario_page_group(domain, label, scenario_num, group, puzzle_text_1, word_1,
                               comp_question, answers_lst, correct_answer, row_num, word_2=None,
                               puzzle_text_2=None,
    ...
    if group == "HD":
        ...
        group_name = "PD"
    #need to update this to link to the DEMO folder in Github
    image_url = "https://github.com/TeachmanLab/MindtrailsMobile_Resources/raw/main/HTC/protocols/MTM/Demo/media/" + \
                label.strip().replace(" ", "_") + "_" + group_name + ".jpeg"
...
def create_resource_page_group_new(motivation_lookup, tip_lst, ER_lookup, domain):
    """
    Create a resource page group (Resource, ER strategy, or Tip)
    :param motivation_lookup: Object created by get_motivation()
    ...
    """
    choices = ["Motivation", "Tip", "ER"]
    resource_type = random.choices(choices, weights=(33, 33, 33), k=1)  # randomly choose one resource type
    if resource_type[0] == "Motivation": #if you get a motivational tip
        motivation = motivation_lookup.pop(0) #take out first list from list of lists
        label = motivation[0] #motivational statement #...
        text = motivation[1] #corresponding text
        motivation_lookup.append(motivation)  # place at back
        text = label + "\n\n" + text
    ...
    resource = [{ #check to see if we still wanna call it resource
        "Name": label,
        "Title": "Reflection",
    ...
    page_group = {
        "Name": "Motivation/Tip/ER",
        "Title": "Motivation/Tip/ER",
        "Type": "Motivation/Tip/ER",
        "DoseSize": 1,
        "Pages": resource,
    }
    return page_group
        ...
        if app_input not in (None, ""):  # if there's an input
            ...
            if app_input == "Picker":
                ...
                add = {
                    ...
                    "Parameters": {
                        "Message": "It’s time to practice thinking flexibly! Head over to Mindtrails Movement for your "
                                   "scheduled session." #change or?
                    }
                }
...
def replace_unicode_char(string):
    string = string.replace("\u2019", "'").replace("\u2013", " - ").replace("\u2014", " - "). \
        replace("\u201c", '"').replace("\u201d", '"').replace("\\n", "\n"). \
        replace("\u2026", "...").replace("\u00e9", "é").replace("\u00e2\u20ac\u2122", "'")
    # "\u00e9" cannot be replaced to é

    # replacing \n with space while keeping \n\n
...
def clean_json(json_dict):
    ...
    if type(json_dict) != dict and type(json_dict) != list:
        if type(json_dict) == str:
            json_dict = replace_unicode_char(json_dict)

        return json_dict

    if type(json_dict) == dict:
        for key in json_dict:
            json_dict[key] = clean_json(json_dict[key])

    if type(json_dict) == list:
        for i in range(len(json_dict)):
            json_dict[i] = clean_json(json_dict[i])
